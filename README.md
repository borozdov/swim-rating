...
### Фронтенд — GitHub Pages, домен `swim-rating.borozdov.ru`

`.github/workflows/pages.yml` публикует `public/` на каждый push в `main`
(там же лежит `public/CNAME` с доменом). Разово в репозитории:
**Settings → Pages → Source → GitHub Actions**, и там же **Custom domain** =
`swim-rating.borozdov.ru`. DNS (Timeweb) — CNAME `swim-rating.borozdov.ru` →
`borozdov.github.io`.

### Бэкенд — Docker на сервере, домен `api.swim-rating.borozdov.ru`

Отдельный поддомен специально для бэкенда, чтобы не конкурировать с GitHub
Pages за один и тот же домен. DNS (Timeweb, без делегирования куда-либо ещё) —
A-запись `api.swim-rating.borozdov.ru` прямо на сервер. Если бэкенд переедет
на другой IP — это единственная запись, которую нужно поменять.
...
`caddy` — обратный прокси на `api.swim-rating.borozdov.ru` (см. `Caddyfile`),
сам получает и продлевает HTTPS-сертификат Let's Encrypt (нужно только, чтобы
DNS уже указывал на сервер и были открыты порты 80/443). `server.js` отдаёт и
...
Если домен фронтенда когда-нибудь изменится — поменять `ALLOWED_ORIGIN` в
[server.js](server.js) и [worker/index.js](worker/index.js). Если домен
бэкенда — `API_ORIGIN` в [public/app.js](public/app.js) и сам `Caddyfile`.
